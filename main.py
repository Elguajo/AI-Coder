...
TEMP_DIR = os.path.join(tempfile.gettempdir(), "chatbot_files")
os.makedirs(TEMP_DIR, exist_ok=True)

# Настройка шаблона подсказки для кодинг-чатбота
PROMPT_TEMPLATE = """
Ты - полезный ассистент по программированию на основе DeepSeek-coder. Ты помогаешь пользователям писать код, объяснять концепции программирования и решать технические проблемы.
...
class CodingChatBot:
    def __init__(self, model_name="deepseek-coder:6.7b-instruct-q4_K_M"):
        """
        Инициализация чат-бота для кодинга
...
def build_interface():
    ...
    with gr.Blocks(title="Локальный чат-бот для кодинга") as interface:
        gr.Markdown("# 🤖 Локальный чат-бот для кодинга")
