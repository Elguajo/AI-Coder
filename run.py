#!/usr/bin/env python3
"""
Главный скрипт для запуска чат-бота с выбором интерфейса
"""
...
def main():
    ...
    """
    parser = argparse.ArgumentParser(description="Локальный чат-бот для кодинга на основе DeepSeek")
    parser.add_argument(
        "--ui", 
    ...
    # Проверка Ollama
    if not check_ollama():
        logger.error("Ollama не запущен. Пожалуйста, запустите Ollama перед запуском чат-бота.")
